#!/usr/bin/env python3
"""
Test script to verify KUKA manipulator mounting on mobile robot.
...
def test_robot_mounting():
    """Test if KUKA manipulator is properly mounted on Husky mobile robot."""
    ...
    # Load KUKA manipulator mounted on top of Husky
    kuka_start_pos = [0, 0, 0.6]  # 0.6m above ground = on top of Husky
    kuka_start_orientation = p.getQuaternionFromEuler([0, 0, 0])
    
    kuka = p.loadURDF("kuka_iiwa/model.urdf",
                      kuka_start_pos,
                      kuka_start_orientation)
    
    print(f"✅ KUKA loaded at position: {kuka_start_pos}")
    ...
    kuka_pos, kuka_orn = p.getBasePositionAndOrientation(kuka)
    
    print(f"\n📍 ACTUAL POSITIONS AFTER PHYSICS:")
    print(f"   Husky: {[round(x, 3) for x in husky_pos]}")
    print(f"   KUKA:  {[round(x, 3) for x in kuka_pos]}")
    
    # Calculate relative position
    rel_height = kuka_pos[2] - husky_pos[2]
    horizontal_dist = np.sqrt((kuka_pos[0] - husky_pos[0])**2 + 
                             (kuka_pos[1] - husky_pos[1])**2)
    
    print(f"\n📏 RELATIVE POSITIONING:")
    print(f"   Height difference: {rel_height:.3f}m")
    print(f"   Horizontal distance: {horizontal_dist:.3f}m")
    
    # Create constraint to mount KUKA on Husky
    constraint_id = p.createConstraint(
        husky, -1,           # Parent: Husky base
        kuka, -1,            # Child: KUKA base
    ...
    kuka_final, _ = p.getBasePositionAndOrientation(kuka)
    
    print(f"\n📍 FINAL POSITIONS WITH CONSTRAINT:")
    print(f"   Husky: {[round(x, 3) for x in husky_final]}")
    print(f"   KUKA:  {[round(x, 3) for x in kuka_final]}")
    
    # Calculate final relative position
    final_rel_height = kuka_final[2] - husky_final[2]
    final_horizontal_dist = np.sqrt((kuka_final[0] - husky_final[0])**2 + 
                                   (kuka_final[1] - husky_final[1])**2)
    ...
    if 0.4 < final_rel_height < 0.6:
        print("   ✅ HEIGHT: KUKA properly mounted on top of Husky")
        height_ok = True
    else:
        print(f"   ❌ HEIGHT: KUKA not at correct height (expected ~0.5m, got {final_rel_height:.3f}m)")
        height_ok = False
    
    if final_horizontal_dist < 0.1:
        print("   ✅ ALIGNMENT: KUKA centered over Husky")
        alignment_ok = True
    else:
        print(f"   ❌ ALIGNMENT: KUKA not centered (distance: {final_horizontal_dist:.3f}m)")
        alignment_ok = False
    
    # Overall assessment
    if height_ok and alignment_ok:
        print("\n🎉 MOUNTING TEST: PASSED")
        print("   The KUKA manipulator is properly mounted on the Husky!")
        success = True
    else:
        print("\n❌ MOUNTING TEST: FAILED")
        print("   The KUKA manipulator is NOT properly mounted.")
        success = False
    
    print(f"\n🔍 VISUAL INSPECTION:")
    print("   Look at the PyBullet GUI window to visually verify mounting")
    print("   - KUKA should be sitting on top of Husky")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("❌ MOUNTING TEST FAILED - KUKA not properly positioned")
        return False
...
def main():
    ...
    if success:
        ...
        print("The KUKA manipulator is properly mounted on the Husky mobile robot.")
    else:
